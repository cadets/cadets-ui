...
function showDetailsPane(node) {
  $.ajax({url: "../filegraph?gnode_id="+node.original.gnode_id,
          type: "GET",
          async: true,
          success: function(data){
              ...
              try {
                ...
                console.log("No file tree info for :" + node.original.gnode_id);
              }
              var id = 'details-pane';
  ...
  DetailsWidget.prototype.createDOM = function(id, node, data) {
    ...
    if(cont_id != this._details_for){
      ...
      if(node.original.type == "default") {
        $.ajax({url: "../provdetail?gnode_id="+(node.original.gnode_id - 2),
                type: "GET",
                async: true,
                success: function(data) {
                  var fdata = convert_to_file_list($.parseJSON(data));
